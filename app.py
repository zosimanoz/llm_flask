...
def generate():
    if request.method == 'GET':
        ...
        if response_type == 'sms':
            ...
            load_correct_model()
            prompt = f'[INST] Generate an email in 300 words for informing customer about {hedis_measure}. Age group: {from_age}-{to_age} [/INST]'
...
def genereate_audio():
    if request.method == 'GET':
        ...
        prepared_response = { 
            "type" : 'message', 
            "data" : f'data saved at {saved_path}'
        } 
...
def play_audio():
    if request.method == 'POST':
        ...
        if request_data['lang'] == 'en':
            audiofile = send_file("output_audio_english.mp3", mimetype="audio/mp3")
        else:
            audiofile = send_file("output_audio_spanish.mp3", mimetype="audio/mp3")
